...
class yolo:
    def __init__(self,weights=ROOT / "yolov5s.pt",  # model path or triton URL
            ...
            self.agnotic_nms = agnostic_nms
            self.max_det = max_det
            device = select_device(device)
            self.model = DetectMultiBackend(weights, device=device, dnn=dnn, data=data, fp16=half)
            self.stride, names, self.pt = self.model.stride, self.model.names, self.model.pt  # check image size
            self.imgsz = imgsz
    def run(self,img):

            imgsz = check_img_size(self.imgsz, s=self.stride) 
            im = letterbox(img, imgsz, stride=self.stride, auto=self.pt)[0]  # padded resize
